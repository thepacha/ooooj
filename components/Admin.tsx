...
export const Admin: React.FC<AdminProps> = ({ user: currentUser }) => {
  ...
  const [sortField, setSortField] = useState<'name' | 'usage' | 'limit' | 'role' | 'lifetime'>('usage');
  ...
  const fetchUsers = async () => {
    ...
    try {
      ...
      const merged: AdminUserView[] = (profiles || []).map(p => {
          ...
          const historicSum = historyData
            ?.filter((h: any) => h.user_id === p.id)
            .reduce((sum: number, h: any) => sum + (h.credits_used || 0), 0) || 0;
                                        ...
                                        {isEditing ? (
                                            ...
                                            <div className="flex justify-end gap-2 opacity-60 group-hover:opacity-100 transition-opacity">
                                                ...
                                                <button 
                                                    ...
                                                    title="Reset Billing Period Usage"
                                                >
                                                    <RotateCcw size={16} />
                                                </button>
                                            </div>
