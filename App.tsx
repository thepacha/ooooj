...
const APP_ROUTES: Record<string, ViewState> = {
  '/dashboard': 'dashboard',
  '/ai-conversation-dashboard': 'ai-conversation-dashboard',
  '/settings': 'settings',
  '/account': 'account',
  '/analyze': 'analyze',
...
function AppContent() {
  ...
  const renderAppView = () => {
    ...
    const isAdminView = ['training', 'assembly-test', 'roster', 'settings', 'admin'].includes(currentView);
  ...
  if (authView === 'careers' && !isRecoveryMode) {
      return (
        <AnimatePresence mode="wait">
          <motion.div 
            key="careers"
  ...
  return (
    <div className={`min-h-screen bg-gradient-to-br from-slate-50 to-slate-100 dark:from-slate-950 dark:to-slate-900 flex font-sans text-slate-900 dark:text-slate-100 print:block print:bg-white print:min-h-0 print:h-auto transition-colors duration-300 ${isRTL ? 'rtl' : ''}`}>
      <Sidebar 
        currentView={currentView === 'evaluation' ? 'history' : (currentView === 'ai-conversation-dashboard' ? 'dashboard' : currentView)} 
